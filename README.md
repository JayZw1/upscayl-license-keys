...
这个文件夹用于上传到你的公开 GitHub 仓库。软件会读取 `day.json`、`month.json`、`year.json`、`century.json` 来判断密钥是否仍在密钥库里。

密钥文件里保存的是密钥的 SHA-256 哈希，不是明文密钥。你可以公开仓库，但不要把生成脚本输出的真实密钥发到公开页面里。

有效期从用户第一次成功输入密钥时开始计算。GitHub 密钥库只负责确认密钥还存在、时长类型是什么、是否 active；软件会在本机保存首次激活时间并计算到期时间。
